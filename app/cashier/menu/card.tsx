...
const MenuCard: React.FC<MenuCardProps> = ({ menu }) => {
  ...
  return (
    <div className="menu-card flex flex-col items-center p-2 border rounded-lg m-2">
      <div className="image-container w-full flex justify-center mb-2">
        {menu.picture ? (
          <Image width={200} height={200} src={`${BASE_IMAGE_MENU}/${menu.picture}`} className="rounded-sm overflow-hidden" alt="preview" unoptimized onError={(e) => (e.currentTarget.src = '/fallback-image.png')} />
        ) : (
          <span>No image</span>
        )}
      </div>
      <div className="image-border w-full border-b-2 border-gray-300 mb-2"></div>
