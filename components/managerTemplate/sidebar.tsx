...
const Sidebar = ({ children, id, title, menuList }: ManagerProp) => {
    ...
    return (
        <div className="w-full min-h-dvh bg-dashboard">
            {/* header section */}
                <header className="flex justify-between items-center p-4 mb-0 bg-[#43576D] shadow-md">
            ...
            {/* end content section */}
            {/* sidebar section */}
                <div className={`backdrop-blur-md flex flex-col w-2/3 md:w-1/2 lg:w-1/4 h-full fixed top-0 right-full transition-transform z-50 bg-gray-400 border-r border-primary rounded-r-[30px] bg-opacity-60 ${isShow ? `translate-x-full sidebar-glow` :``}`}>
                    {/* close button */}
