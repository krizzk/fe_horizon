...
const CashierTemplate = ({ children, id, title, menuList, user }: CashierProp) => {
  ...
  useEffect(() => {
    ...
    const picture = getCookie("profile_picture")
        ...
        <div className="p-4 border-b">
          <div className="flex items-center space-x-3">
            <div className="relative h-10 w-10 rounded-full overflow-hidden border-2 border-gray-200">
              {profilePicture ? (
                <Image src={`${BASE_IMAGE_PROFILE}/${profilePicture}`} alt="Profile" fill className="object-cover" />
              ) :
               (
                <Image src={`${BASE_IMAGE_PROFILE}/${profilePicture}`} alt="Default Profile" fill className="object-cover" />
              )
              }
            </div>
